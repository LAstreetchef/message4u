...
Secret Message is a playful web application that enables users to send paywalled messages. Senders create messages with custom pricing, and recipients must pay the specified amount to unlock and view the message content. The platform acts as a payment processor, collecting payments via Stripe and cryptocurrency (NOWPayments), deducting a platform fee ($1.69 + 6.9% of the message price), and facilitating automated payouts to senders via Stripe Connect for bank transfers. The platform also supports cryptocurrency payments via NOWPayments (200+ cryptocurrencies) and manual Venmo/CashApp payouts as fallback options. The application features an Instagram-inspired dark mode aesthetic with deep black backgrounds, high-contrast white text, purple-pink gradient accents, and rounded pill-shaped elements.

The platform supports authenticated senders using password-based authentication (email + password sign-up and login) who can create and manage multiple paywalled messages, connect their bank accounts via Stripe Connect for automated payouts, and track earnings after platform fees. Recipients can access messages without requiring an account and can pay using credit cards (Stripe) or cryptocurrency (NOWPayments). Messages can be either text-based (converted to images for privacy) or file uploads (any file type up to 10MB), both protected behind payment. Files are stored securely in Replit Object Storage with ACL-based access control. When recipients provide a valid email address, they automatically receive beautifully styled email notifications with direct links to unlock their messages.
...
- `/api/stripe/create-connect-account` - Create Stripe Express account for automated payouts
- `/api/stripe/create-account-link` - Generate Stripe onboarding link for bank account connection
- `/api/stripe/connect-status` - Check Stripe Connect onboarding and payout eligibility status
- `/api/messages` - CRUD operations for messages
- `/api/create-payment-intent` - Stripe checkout session creation
- `/api/create-crypto-payment` - NOWPayments cryptocurrency payment creation
- `/api/messages/:slug/check-payment` - Payment verification with platform fee calculation (Stripe and crypto)
...
- `/api/webhook/stripe` - Stripe webhook handler for payment and Connect account events
- `/api/webhook/nowpayments` - NOWPayments webhook handler for crypto payment confirmations
...
**Payment Processing**: Dual payment gateway integration supporting both credit cards (Stripe) and cryptocurrency (NOWPayments with 200+ supported coins). The platform acts as the payment processor, collecting the full payment amount from recipients. A platform fee of $1.69 + 6.9% of the message price is calculated using integer cents arithmetic to ensure penny-accurate calculations. Sender earnings (amount minus platform fee) are tracked in the database. The flow uses server-side session creation and client-side redirection, with payment verification on the unlocked page.

**Automated Payouts**: Stripe Connect integration enables automated bank transfers to senders. Senders complete a one-time Stripe Express onboarding to connect their bank account. The platform administrator can trigger automated payouts from the admin dashboard by clicking "Complete Payout" on any paid message. The system creates a Stripe Transfer to the sender's connected account, automatically deducting the platform fee. Transfer status is tracked via webhooks (transfer.created, transfer.paid). Manual payouts via Venmo, CashApp, or cryptocurrency wallet addresses remain available as fallback options for senders who prefer not to use Stripe Connect.
...
**Schema Design**:
- `users` - Stores user authentication and profile data (id, email, passwordHash, payoutMethod, payoutAddress, cryptoWalletAddress, cryptoWalletType, stripeAccountId, stripeOnboardingComplete, createdAt, updatedAt)
...
- `payments` - Payment transaction records with platform fee tracking (id, messageId, amount, stripeSessionId, nowpaymentsPaymentId, platformFee, senderEarnings, payoutStatus, stripeTransferId, createdAt)
...
**Payment Gateway**: Dual integration with Stripe and NOWPayments
- Stripe - Processes credit card payments, manages checkout sessions, and handles automated payouts via Stripe Connect Express accounts. Uses test mode for development. Environment variables: `STRIPE_SECRET_KEY` (server), `VITE_STRIPE_PUBLIC_KEY` (client), `STRIPE_WEBHOOK_SECRET` (webhook signature verification).
- NOWPayments - Processes cryptocurrency payments with support for 200+ cryptocurrencies. Environment variables: `NOWPAYMENTS_API_KEY` (server), `VITE_NOWPAYMENTS_API_KEY` (client), `NOWPAYMENTS_IPN_SECRET` (webhook signature verification).
